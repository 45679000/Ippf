 ...
 */
(function () {
	var WebDataRocksHighcharts = {};
	window["WebDataRocksHighcharts"] = WebDataRocksHighcharts;
	...
	function prepareChartInfo(data, options) {
		...
		if (options.type != undefined) output.chart = {
			type: options.type
		};
	...
	function prepareSeries(output, data) {
		...
		for (var i = 0; i < data.data.length; i++) {
			...
			for (var j = 0; j < data.meta.vAmount; j++) {
				...
				if (series["_" + _yName][_seriesName] == undefined) {
					series["_" + _yName][_seriesName] = [];
				}
				value = isNaN(record["v" + j]) ? null : record["v" + j];
				series["_" + _yName][_seriesName].push(value);
			}
		}
	...
	function prepareSeriesDatetime(output, data) {
		...
		for (var i = 0; i < data.data.length; i++) {
			...
			for (var j = 0; j < data.meta.vAmount; j++) {
				...
				value = isNaN(record["v" + j]) ? null : record["v" + j];
				series["_" + _yName][_seriesName].push([(record["r0"] != undefined ? record["r0"] : record["c0"]), value]);
			}
		}
		//output.xAxis.categories = categories;
	...
	function prepareSeriesWithDrilldown(output, data) { //can drill be for more than one level?
		...
		for (var i = 0; i < data.data.length; i++) {
			...
			if (data.meta["rAmount"] > 0 && data.meta["cAmount"] > 0) {
				...
				if (record["c0"] == undefined) {
					value = isNaN(record["v0"]) ? null : record["v0"];
					mainSeries[data.meta["r0Name"]].push({
						name: record["r0"],
						y: value,
						drilldown: record["r0"]
					});
		...
		for (var mainseriesname in mainSeries) {
			output.series.push({
				"name": mainseriesname,
				colorByPoint: true,
				"data": mainSeries[mainseriesname]
			});
		}
